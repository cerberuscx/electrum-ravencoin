...
info "Installing dependencies specific to ravencoin..."
python3 -m pip install --no-dependencies --no-warn-script-location -Ir ./contrib/deterministic-build/requirements-ravencoin.txt \
    || fail "Could not install dependencies specific to ravencoin"

info "Installing dependencies specific to ravencoin binaries..."
# We don't want to ignore installed packages
python3 -m pip install --no-warn-script-location -r ./contrib/deterministic-build/requirements-ravencoin-binaries.txt \
    || fail "Could not install dependencies specific to ravencoin binaries"
